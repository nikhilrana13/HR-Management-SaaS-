...
export const GetAllEmployeeLeaves = async(req,res)=>{
     try {
         ...
         const leaves = await Leave.find({companyId:company._id}).populate("employeeId","name email profilepic department position").populate("approvedBy","name").populate("rejectedBy","name").skip(skip).limit(limit).sort({createdAt:-1}).lean()
