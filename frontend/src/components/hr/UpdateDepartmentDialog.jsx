...
const UpdateDepartmentDialog = ({ deptid, deptname,setDepartments}) => {
    ...
    const handleUpdateDepartment = async () => {
        ...
        try {
            setLoading(true)
            const response = await axios.put(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/department/update/${deptid}`,{
                name:input.name
            },{
                headers:{
            ...
            if(response?.data?.status === "success"){
                ...
                const updatedname = response?.data?.data?.department?.name
                setDepartments((prev)=> prev.map((dept)=> dept._id === deptid ? {...dept,name:updatedname}:dept))
            }
            setOpen(false)
        } catch (error) {
    ...
    return (
        <>
            <CiEdit onClick={() => setOpen(true)} size={25} />
            {/* MODAL */}
            {open && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 overflow-y-auto">
                    ...
                    <div onClick={(e) => e.stopPropagation()} className="relative  w-full max-w-lg bg-white dark:bg-[#161a2d] rounded-2xl shadow-2xl overflow-hidden">
                        ...
                        <div className="px-6 py-4 bg-slate-50 dark:bg-slate-800/50 flex items-center justify-end gap-3">
                            <button
                                onClick={() => {setOpen(false),setInput({name:deptname}),setError({})}}
                                type="button"
                            ...
                            <button
                               onClick={()=>handleUpdateDepartment()}
                                className="px-4 cursor-pointer py-2.5 rounded-lg text-sm font-bold bg-[#1337ec] text-white hover:bg-[#1337ec]/90 shadow-md shadow-[#1337ec]/20 transition-all"
                            >
                                {loading ? (
                                    <ThreeDots
                                            visible={true}
